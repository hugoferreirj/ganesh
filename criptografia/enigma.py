...
rotor1 = ['J', 'P', 'S', 'X', 'V', 'U', 'T', 'C', 'R', 'Z', 'W', 'O',
          'N', 'Y', 'L', 'K', 'A', 'I', 'G', 'H', 'F', 'E', 'D', 'B', 'M', 'Q']

rotor2 = ['Q', 'W', 'E', 'V', 'C', 'X', 'Z', 'L', 'K', 'J', 'H', 'G',
          'F', 'D', 'S', 'A', 'P', 'O', 'I', 'U', 'Y', 'T', 'R', 'B', 'N', 'M']

reflector = {'A': 'S', 'M': 'V', 'J': 'Z', 'L': 'Q', 'C': 'O', 'H': 'U', 'G': 'W', 'F': 'P', 'K': 'R', 'I': 'N', 'E': 'T', 'D': 'X',
             'B': 'Y', 'S': 'A', 'V': 'M', 'Z': 'J', 'Q': 'L', 'O': 'C', 'U': 'H', 'W': 'G', 'P': 'F', 'R': 'K', 'N': 'I', 'T': 'E', 'X': 'D', 'Y': 'B'}
...
def encrypt(message, init_pos: tuple):
    ...
    for pos, rotor in zip(init_pos, (n_rotor1, n_rotor2)):
        assert (type(pos) == int) and (
            1 <= pos <= 26), "A posição inicial do rotor deve ser entre 1 e 26"
        pos -= 1
        for _ in range(pos):
            turn(rotor)
...
def mappings():
    letter = 'U'
    print(
        f"Resultado de encriptar/decriptar a letra {letter} com todas as posições iniciais:")
